...
public class Toolkit {
    ...
    public static void loadGLOVE() throws IOException {
        try {
            ...
            while ((line = myReader.readLine()) != null) {
                String[] new_array = new String[51];
                double[] vec_array = new double[50];
                new_array = line.split(",");
                int k = 0;
                listVocabulary.add(new_array[0]);
                for (int i = 1; i <= 50; i++) {
                    vec_array[k] = Double.parseDouble(new_array[i]);
                    k++;
                }
                listVectors.add(vec_array);
    ...
    public static List<String> loadStopWords() throws IOException {
        ...
        try {
            BufferedReader myReader = new BufferedReader(new FileReader(getFileFromResource(FILENAME_STOPWORDS)));
            String line;
            while ((line = myReader.readLine()) != null) {
